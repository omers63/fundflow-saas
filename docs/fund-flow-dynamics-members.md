...
```mermaid
flowchart LR
    BANK["Your bank"] -->|"You transfer"| POOL["Fund accepts deposit"]
    POOL --> CASH["Your cash balance ↑"]
    CASH --> AUTO["System may auto-collect<br/>contribution or loan EMI"]
```
...
```mermaid
flowchart LR
    CASH["Cash balance"] -->|"Collection"| FUND["Fund balance ↑"]
```
...
```mermaid
flowchart TD
    FUND["Fund balance ↓<br/><i>your portion</i>"] --> LOAN["Loan approved"]
    LOAN --> CASH["Cash balance ↑<br/><i>loan amount</i>"]
    CASH --> REQ["You request cash-out"]
    REQ --> BANK["Money sent to your bank"]
```
...
```mermaid
flowchart LR
    PAY["You pay in"] --> CASH["Cash ↑ then collected"]
    CASH --> FUND["Fund ↑"]
    FUND --> LOAN["Loan balance ↓"]
```
