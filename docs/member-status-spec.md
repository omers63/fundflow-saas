...
stateDiagram-v2
    ...
    active --> inactive: Freeze (member request approved or admin)
    inactive --> active: Unfreeze (request approved or admin)

    active --> delinquent: Arrears breach (auto/manual)
    delinquent --> active: Arrears cleared (auto/restore)
    delinquent --> active: Force restore (admin)

    active --> suspended: Admin suspend
    delinquent --> suspended: Admin suspend
    suspended --> active: Restore (no arrears)
    suspended --> delinquent: Restore (arrears remain)

    active --> withdrawn: Withdraw request approved / admin
    inactive --> withdrawn: Admin withdraw
    delinquent --> withdrawn: Admin withdraw
    suspended --> withdrawn: Admin withdraw

    active --> terminated: Admin terminate
    inactive --> terminated: Admin terminate
    delinquent --> terminated: Admin terminate
    suspended --> terminated: Admin terminate

    withdrawn --> active: Reinstate (balances cleared)
    terminated --> active: Reinstate (balances cleared + payout unfrozen)

    note right of suspended
      Guarantor transfer:
      suspended + contribution_cycles_active=true
    end note
```
