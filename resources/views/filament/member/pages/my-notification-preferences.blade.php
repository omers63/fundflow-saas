<x-filament-panels::page>
    ...
    <div
        ...
        <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-4">
            {{ __('Available Communication Channels') }}</h3>
    ...
    <div class="space-y-3">
        ...
        <div class="hidden lg:grid lg:grid-cols-12 gap-4 px-5 pb-1">
            <div class="col-span-4 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
                {{ __('Notification Category') }}</div>
            <div
                class="col-span-8 grid grid-cols-4 gap-2 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400 text-center">
                <div>🔔 {{ __('In-App') }}</div>
                <div>✉️ {{ __('Email') }}</div>
                <div>📱 {{ __('SMS') }}</div>
                <div>💬 {{ __('WhatsApp') }}</div>
        ...
        @foreach ($this->categories as $type => $meta)
            @php
                ...
                $channelLabels = ['in_app' => __('In-App'), 'email' => __('Email'), 'sms' => __('SMS'), 'whatsapp' => __('WhatsApp')];
    ...
    <div class="mt-4 rounded-lg bg-blue-50 dark:bg-blue-950/30 ring-1 ring-blue-200 dark:ring-blue-800/40 p-4">
        <p class="text-xs text-blue-700 dark:text-blue-300 leading-relaxed">
            ...
            {!! __('Make sure your phone number is set in <a href=":url" wire:navigate class="underline">My Profile</a> to receive SMS and WhatsApp notifications.', ['url' => \App\Filament\Member\Pages\MyProfilePage::getUrl(panel: 'member')]) !!}
