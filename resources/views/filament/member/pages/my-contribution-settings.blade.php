<x-filament-panels::page>
    ...
    <div
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-4xl sm:text-5xl font-extrabold tracking-tight">{{ __('SAR') }}
                    {{ number_format($current) }}</p>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 mb-6">
        <div
            ...
            <p class="mt-0.5 text-xs text-gray-400 pl-2">
                {{ now()->locale(app()->getLocale())->translatedFormat('F Y') }}</p>
        </div>
        ...
        <div
            ...
            <p class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400 pl-2">
                {{ __('Total Contributed') }}</p>
            <p class="mt-1 text-2xl font-bold text-gray-900 dark:text-white pl-2">{{ __('SAR') }}
                {{ number_format($totalEver) }}</p>
    ...
    <x-filament::section>
        ...
        <x-slot name="description">
            {{ __('Multiples of SAR 500, from :min to :max.', ['min' => __('SAR') . ' ' . number_format($minOpt), 'max' => __('SAR') . ' ' . number_format($maxOpt)]) }}
