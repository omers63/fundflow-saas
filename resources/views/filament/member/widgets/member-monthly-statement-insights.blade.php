            ...
            <div class="grid grid-cols-3 divide-x divide-gray-100 dark:divide-gray-700 sm:grid-cols-6">
                @foreach ($kpis as $i => $card)
                    ...
                    <div class="ff-app-insights-kpi ff-member-stat-card relative px-2.5 py-2 transition"
                        ...
                        <p class="mt-0.5 pl-1 text-[10px] font-medium uppercase tracking-wide text-gray-500">{{ ui_label($card['label']) }}</p>
                        <p class="pl-1 text-lg font-bold tabular-nums leading-tight text-gray-900 dark:text-white">{{ $card['value'] }}</p>
                ...
                <div class="flex h-20 items-end gap-1.5 sm:gap-2">
                    @foreach ($d['trend'] as $month)
                        ...
                        <div class="flex flex-1 flex-col items-center gap-0.5">
                            <span class="text-[10px] font-semibold tabular-nums text-gray-500">{{ number_format($month['closing'], 0) }}</span>
