    ...
    <div class="ff-app-insights w-full max-w-none space-y-3 mb-1">
        ...
        'border-gray-200/70 bg-gradient-to-r from-gray-50 to-slate-50/60 dark:border-gray-600/25 dark:from-gray-950/30 dark:to-slate-950/20' => $latest === null,
            ...
            <div
                ...
                <div
                    class="grid min-w-0 grid-cols-1 gap-2 divide-x divide-gray-100 dark:divide-gray-700 sm:grid-cols-2 lg:grid-cols-3">
            ...
            : (string) ($card['value'] ?? '—');
                        ...
                        <div class="ff-app-insights-kpi ff-member-stat-card relative min-w-0 px-2.5 py-2 transition"
                            ...
                            <x-ff-stat-line :text="$labelText"
                                class="mt-0.5 break-words pl-1 text-[10px] font-medium uppercase tracking-wide text-gray-500" />
                            @if (isset($card['amount']))
                                <x-ff-stat-line :amount="$card['amount']" :currency="$currency" :precision="$card['precision'] ?? 2"
                                    class="break-words pl-1 text-lg font-bold tabular-nums leading-tight text-gray-900 dark:text-white" />
                            @else
                                <x-ff-stat-line :text="$valueText"
                                    class="break-words pl-1 text-lg font-bold tabular-nums leading-tight text-gray-900 dark:text-white" />
                            @endif
                            <x-ff-stat-line :text="$subText" class="break-words pl-1 text-[10px] text-gray-400" />
                        </div>
                    @endforeach
                </div>
                @if (collect($d['sparkline'])->sum() > 0)
                    <div class="flex h-5 items-end gap-px border-t border-gray-100 px-2 py-1 dark:border-gray-700" dir="ltr">
                ...
                @endif
                    ...
                    @if (count($d['trend']) > 0)
                                <div
                                    ...
                                    <div class="px-3 py-3">
                                        <div class="flex h-20 items-end gap-1.5 sm:gap-2" dir="ltr">
