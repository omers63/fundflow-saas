            ...
            $bar = $accentBar[$card['accent']] ?? 'bg-gray-400';
                ...
                'pointer-events-none opacity-60' => empty($card['url']),
                    ...
                    <div class="flex items-center gap-1.5 pl-1">
                        <x-dynamic-component :component="$card['icon']" class="h-3.5 w-3.5 text-gray-400 dark:text-gray-500" />
                        ...
                        @if (filled($card['hint'] ?? null))
                            <x-ff-stat-line :text="ui_label($card['hint'])" class="truncate pl-1 text-[10px] text-gray-400 dark:text-gray-500" />
                        @endif
                </a>
        @endforeach
    </div>
