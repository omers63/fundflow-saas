        ...
        $bar = $accentBar[$card['accent']] ?? 'bg-gray-400';
            ...
            'pointer-events-none opacity-60' => empty($card['url']),
                        ...
                        <div class="flex items-center gap-1.5 pl-1">
                            <x-dynamic-component :component="$card['icon']" class="h-3.5 w-3.5 shrink-0 text-gray-400 dark:text-gray-500" />
                            <x-ff-stat-line :text="ui_label($card['label'])"
                                class="min-w-0 break-words text-[10px] font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400" />
                            </div>
                            <x-ff-stat-line :text="(string) $card['value']"
                                class="mt-1 break-words pl-1 text-sm font-bold text-gray-900 dark:text-white" />
                            @if (filled($card['hint'] ?? null))
                                <x-ff-stat-line :text="ui_label($card['hint'])" class="break-words pl-1 text-[10px] text-gray-400 dark:text-gray-500" />
                            @endif
                    </a>
        @endforeach
    </div>
