                                ...
                                <div class="mt-3 space-y-3 border-t border-gray-100 pt-3 dark:border-white/10">
                                    ...
                                    @forelse ($sections as $section)
                                        @php
                                            ...
                                            $sectionOpen = (bool) ($section['default_open'] ?? false);
                                            $rowCount = count($section['rows'] ?? []);
                                        @endphp

                                        @if ($isCollapsible)
                                            <details
                                                class="group rounded-lg border border-gray-100 dark:border-white/10"
                                            ...
                                            >
                                                <summary class="cursor-pointer list-none px-3 py-2.5 [&::-webkit-details-marker]:hidden">
                                                    <div class="flex items-center justify-between gap-2">
                                                        <div class="min-w-0">
                                                            ...
                                                            <p class="mt-0.5 text-[11px] text-gray-500 dark:text-gray-400">
                                                                {{ trans_choice(':count row|:count rows', $rowCount, ['count' => number_format($rowCount)]) }}
                                                                @if ($section['truncated'])
                                                                    · {{ __('truncated in snapshot') }}
                                                                @endif
                                                            </p>
                                                        </div>
                                                        <x-heroicon-m-chevron-down class="h-4 w-4 shrink-0 text-gray-400 transition group-open:rotate-180" />
                                                ...
                                                <div class="border-t border-gray-100 px-3 py-3 dark:border-white/10">
                                                    @include('filament.tenant.partials.reconciliation.snapshot-detail-section-body', [
                                                        'section' => $section,
                                                        'currency' => $currency,
                                                    ])
                                                </div>
