                    ...
                    <div
                        ...
                        <div class="grid grid-cols-1 divide-y divide-gray-100 dark:divide-gray-800 sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-4">
                            <a href="{{ $pipeline['members_active_url'] }}"
                                ...
                                <p class="mt-0.5 text-xl font-bold tabular-nums text-emerald-600 dark:text-emerald-400">
                                    {{ $d['active'] }}
                                </p>
                            </a>
                            <a href="{{ $pipeline['members_inactive_url'] ?? MemberResource::listTabUrl('inactive') }}"
                                class="px-3 py-2.5 text-center transition hover:bg-gray-50/80 dark:hover:bg-gray-800/60">
                                <p class="text-[10px] font-semibold uppercase tracking-wider text-gray-500">{{ __('Inactive') }}
                                </p>
                                <p class="mt-0.5 text-xl font-bold tabular-nums text-gray-700 dark:text-gray-200">
                                    {{ $d['inactive'] }}
                                </p>
                            </a>
                            <a href="{{ $pipeline['members_withdrawn_url'] ?? MemberResource::listTabUrl('withdrawn') }}"
                                class="px-3 py-2.5 text-center transition hover:bg-rose-50/60 dark:hover:bg-rose-950/20">
                                <p class="text-[10px] font-semibold uppercase tracking-wider text-gray-500">{{ __('Withdrawn') }}
                                </p>
                                <p class="mt-0.5 text-xl font-bold tabular-nums text-rose-600 dark:text-rose-400">
                                    {{ $d['withdrawn'] }}
                                </p>
                            </a>
                            <a href="{{ $pipeline['members_migration_url'] ?? MemberResource::listTabUrl('migration_pending') }}"
                                class="px-3 py-2.5 text-center transition hover:bg-violet-50/60 dark:hover:bg-violet-950/20">
                                <p class="text-[10px] font-semibold uppercase tracking-wider text-gray-500">{{ __('Migration') }}
                                </p>
                                <p class="mt-0.5 text-xl font-bold tabular-nums text-violet-600 dark:text-violet-400">
                                    {{ $d['migration_pending'] ?? 0 }}
                                </p>
                            </a>
                        </div>
                        ...
                        <div class="border-t border-gray-100 px-3 py-2 dark:border-gray-800">
                            ...
                            <div class="mt-1.5 flex flex-wrap gap-2 text-xs text-gray-600 dark:text-gray-300">
                                <span
                        ...
                        <div class="divide-y divide-gray-100 dark:divide-gray-800">
                            @forelse ($d['attention_queue'] as $member)
                                <a href="{{ $member['view_url'] }}"
                                    ...
                                    <span
                                        class="flex h-7 w-7 shrink-0 items-center justify-center rounded-xl bg-amber-100 text-xs font-bold text-amber-800 dark:bg-amber-900/40 dark:text-amber-200">
