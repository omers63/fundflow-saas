    ...
    <div class="overflow-hidden rounded-xl border border-gray-200/80 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
        <div class="flex items-center justify-between gap-2 border-b border-gray-100 px-3 py-2 dark:border-gray-700">
            <div class="flex items-center gap-1.5">
                ...
                <h3 class="text-[11px] font-semibold uppercase tracking-wider text-gray-500">{{ __('Queue stages') }}</h3>
            </div>
        </div>
        <div class="grid grid-cols-3 divide-x divide-gray-100 dark:divide-gray-700">
            <a href="{{ $pipeline['queue_url'] }}?tab=needs_decision" @class(['flex flex-col items-center px-2 py-3 text-center transition', 'bg-amber-50/80 dark:bg-amber-950/30' => $d['active_tab'] === 'needs_decision'])>
            ...
            <a href="{{ $pipeline['queue_url'] }}?tab=ready_to_disburse" @class(['flex flex-col items-center px-2 py-3 text-center transition', 'bg-sky-50/80 dark:bg-sky-950/30' => $d['active_tab'] === 'ready_to_disburse'])>
            ...
            <a href="{{ $pipeline['queue_url'] }}?tab=awaiting_payout" @class(['flex flex-col items-center px-2 py-3 text-center transition', 'bg-indigo-50/80 dark:bg-indigo-950/30' => $d['active_tab'] === 'awaiting_payout'])>
    ...
    <div class="overflow-hidden rounded-xl border border-gray-200/80 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
        <div class="flex items-center justify-between gap-2 border-b border-gray-100 px-3 py-2 dark:border-gray-700">
            <div class="flex items-center gap-1.5">
                ...
                <h4 class="text-[11px] font-semibold uppercase tracking-wider text-gray-500">{{ $d['tab_labels'][$d['active_tab']] ?? __('Queue') }}</h4>
            </div>
        </div>
        <div class="divide-y divide-gray-100 dark:divide-gray-700">
            @forelse ($d['preview'] as $loan)
                <a href="{{ $loan['view_url'] }}" class="flex items-center gap-2 px-3 py-2 transition hover:bg-gray-50 dark:hover:bg-gray-800/80">
                    <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-sky-100 text-[10px] font-bold text-sky-800 dark:bg-sky-900/40">
                        {{ $loan['queue'] ? '#'.$loan['queue'] : '·' }}
                    </span>
                    <div class="min-w-0 flex-1">
                        <p class="truncate text-xs font-medium text-gray-900 dark:text-white">{{ $loan['member'] }}</p>
                        <p class="truncate text-[10px] text-gray-400">{{ number_format($loan['amount'], 0) }} {{ $currency }} · {{ $loan['fund_tier'] ?? '—' }}</p>
                    </div>
