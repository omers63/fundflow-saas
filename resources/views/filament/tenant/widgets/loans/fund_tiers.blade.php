    ...
    <div class="flex items-center justify-between gap-2 border-b border-gray-100 px-3 py-2 dark:border-gray-700">
        <div class="flex items-center gap-1.5">
            ...
            <h3 class="text-[11px] font-semibold uppercase tracking-wider text-gray-500">
                {{ __('Pool utilization by tier') }}</h3>
        </div>
        <a href="{{ $d['queue_url'] }}"
            class="text-[10px] font-semibold text-sky-600 hover:underline">{{ __('Open queue') }}</a>
    </div>
        ...
        @foreach ($d['breakdown'] as $tier)
            <div class="px-3 py-2.5">
                    ...
                    <div class="min-w-0">
                        <p class="text-xs font-semibold text-gray-900 dark:text-white">
                            ...
                            @if ($tier['is_emergency'])
                                <span
                                    class="ml-1 rounded bg-rose-100 px-1 py-0.5 text-[9px] font-bold uppercase text-rose-700 dark:bg-rose-900/40">{{ __('Emergency') }}</span>
                            @endif
                        </p>
                        <p class="text-[10px] text-gray-400">{{ $tier['loan_tier'] ?? __('—') }} ·
                            {{ $tier['percentage'] }}%</p>
                    </div>
                ...
                <div class="mt-1 flex justify-between text-[10px] text-gray-500">
                    <span>{{ __('Deployed') }}: <x-member::amount :value="$tier['exposure']" :currency="$currency"
                            :precision="0" class="inline" /></span>
                    <span class="text-emerald-600">{{ __('Available') }}: <x-member::amount :value="$tier['available']"
                            :currency="$currency" :precision="0" class="inline" /></span>
                </div>
            </div>
        @endforeach
    </div>
