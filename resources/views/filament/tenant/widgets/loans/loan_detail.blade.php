...
<div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
    @foreach ($d['progress'] as $key => $bar)
        <div class="rounded-xl border border-gray-200/80 bg-white px-3 py-2.5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <div class="h-2 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-700">
                <div @class(['h-full rounded-full', 'bg-indigo-500' => $key === 'disburse', 'bg-teal-500' => $key === 'repay']) style="width: {{ max(4, $bar['percent']) }}%"></div>
            </div>
        </div>
    @endforeach
</div>
...
@if ($d['next_due'])
    <div class="rounded-xl border border-amber-200/80 bg-gradient-to-r from-amber-50 to-orange-50/60 px-3 py-2.5 shadow-sm dark:border-amber-500/30 dark:from-amber-950/30 dark:to-orange-950/20">
        <div class="flex flex-wrap items-center justify-between gap-2">
            <div class="flex items-center gap-2">
                <x-heroicon-o-calendar-days class="h-4 w-4 text-amber-600" />
                <div>
                    <p class="text-[10px] font-semibold uppercase tracking-wider text-amber-800 dark:text-amber-200">{{ __('Next installment') }}</p>
                    <p class="text-sm font-bold text-gray-900 dark:text-white">{{ $d['next_due']['amount'] }} · {{ $d['next_due']['date'] }}</p>
                </div>
            </div>
...
<div class="grid grid-cols-1 gap-2 sm:grid-cols-3">
    @foreach ($d['relation_summaries'] as $card)
        ...
        <div class="relative overflow-hidden rounded-xl border border-gray-200/80 bg-white px-3 py-2.5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <div class="flex items-center gap-1.5 pl-1">
                ...
                <p class="text-[10px] font-semibold uppercase tracking-wider text-gray-500">{{ ui_label($card['label']) }}</p>
            </div>
...
@if ($d['guarantor'])
    <div class="rounded-xl border border-gray-200/80 bg-white px-3 py-2 text-[11px] shadow-sm dark:border-gray-700 dark:bg-gray-800">
