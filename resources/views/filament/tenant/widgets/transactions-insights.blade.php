...
@component('filament.tenant.partials.ops-overview.shell', [
    ...
    @else
        ...
        <div
            class="w-full overflow-hidden rounded-xl border border-gray-200/80 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
            <div
                ...
                <div class="flex items-center gap-1.5">
                    ...
                    <h3 class="text-[11px] font-semibold uppercase tracking-wider text-gray-500">
                        {{ $d['trend_title'] ?? __('Flow trend') }}</h3>
                </div>
                <div class="text-[10px] text-gray-400">
                    @if (! empty($d['trend_period_label']))
                        <span>{{ $d['trend_period_label'] }}</span>
                        <span aria-hidden="true"> · </span>
                    @endif
                    {{ __('Credits') }} {!! \App\Support\Insights\InsightFormatter::moneyMarkup($d['summary']['credits_total']) !!}
                    ·
                    {{ __('Debits') }} {!! \App\Support\Insights\InsightFormatter::moneyMarkup($d['summary']['debits_total']) !!}
                </div>
            </div>
            <div class="px-3 py-3">
                <div class="flex h-32 w-full items-end gap-0.5 sm:gap-1" dir="ltr">
                    @php $trendLabelInterval = (int) ($d['trend_label_interval'] ?? 6); @endphp
                    @foreach ($d['trend'] as $day)
                        ...
                        <div class="flex min-w-0 flex-1 flex-col items-center gap-1"
                            title="{{ $day['label'] }} · {{ __('Flow') }} {{ \App\Support\Insights\InsightFormatter::money($day['flow_total']) }}">
                            ...
                            @if ($loop->first || $loop->last || $loop->iteration % $trendLabelInterval === 0)
                                <span class="max-w-full truncate text-[8px] text-gray-400">{{ $day['label'] }}</span>
                            @else
                ...
                <div class="mt-3 grid grid-cols-1 gap-2 md:grid-cols-3">
                    <div class="rounded-lg bg-gray-50 px-3 py-2 dark:bg-gray-900/30">
                        ...
                        ])>{!! \App\Support\Insights\InsightFormatter::moneyMarkup($d['summary']['net_total']) !!}</p>
                    </div>
                    <div class="rounded-lg bg-gray-50 px-3 py-2 dark:bg-gray-900/30">
                        <p class="text-[10px] font-semibold uppercase tracking-wider text-gray-500">
                            {{ __('Linked source') }}</p>
