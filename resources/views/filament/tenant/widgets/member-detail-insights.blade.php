...
<div class="ff-app-insights w-full max-w-none space-y-3 mb-1" @if (filled($pollingInterval)) wire:poll.{{ $pollingInterval }} @endif>
    ...
    @else
        ...
        >
            ...
            <div class="grid gap-4 border-t border-gray-200/80 px-4 py-4 dark:border-white/10 md:grid-cols-[1.15fr_0.85fr] md:items-center">
                <div class="min-w-0">
                    <p @class([
                    ...
                    @elseif ($d['loan'] === null)
                        <div class="rounded-xl border border-dashed border-gray-200/90 px-3 py-2.5 dark:border-white/10">
                            ...
                            @if (! $d['eligibility']['eligible'] && filled($d['eligibility']['reason'] ?? null))
                                <p class="mt-0.5 text-[11px] text-gray-500 dark:text-gray-400">{{ $d['eligibility']['reason'] }}</p>
                            @endif
                        </div>
                    @endif
                </div>
            ...
            @if (($d['arrears']['visible'] ?? false) || filled($snapshot['status_cta_url'] ?? null) || $metrics !== [])
        ...
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
            ...
            <div class="overflow-hidden rounded-xl border border-gray-200/80 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                ...
                <ul class="divide-y divide-gray-100 dark:divide-gray-700">
                    ...
                    @empty
                        <li class="px-3 py-4 text-center text-[11px] text-gray-400">{{ __('No ledger activity yet') }}</li>
                    @endforelse
                </ul>
            </div>
        </div>
