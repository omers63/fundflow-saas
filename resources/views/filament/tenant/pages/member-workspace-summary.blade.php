    ...
    <section
        ...
        <div class="space-y-3 p-3">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                ...
                <div class="shrink-0 text-[11px] text-gray-500 dark:text-gray-400 sm:max-w-[11rem] sm:text-end">
                    <p class="truncate" title="{{ $summary['monthly_formatted'] ?? '—' }}">
                        {{ __(':monthly monthly', ['monthly' => $summary['monthly_formatted'] ?? '—']) }}
                    </p>
                </div>
            </div>
                ...
                @if ($loan !== null)
                    <div class="flex flex-wrap items-center justify-between gap-2 text-xs">
                        ...
                        @if (filled($loan['url'] ?? null))
                            <a href="{{ $loan['url'] }}" class="shrink-0 font-medium text-indigo-600 hover:underline dark:text-indigo-400">
                                {{ __('View loans') }}
                            </a>
                        @endif
                    </div>
