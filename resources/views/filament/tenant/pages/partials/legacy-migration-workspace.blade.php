<div class="ff-legacy-migration mx-auto w-full max-w-5xl space-y-6">
    ...
    {{-- Step 1: Members --}}
    @if ($currentStep === 1)
        <section class="ff-maintenance-panel" @if ($membersImportRunning) wire:poll.2s="pollWizardStepStatus" @endif>
    ...
    @endif
        ...
        {{-- Step 2: Loans --}}
        @if ($currentStep === 2)
            <section class="ff-maintenance-panel" @if ($loansImportRunning) wire:poll.2s="pollWizardStepStatus" @endif>
        ...
        {{-- Step 3: Classify payments --}}
        @if ($currentStep === 3)
            <section class="ff-maintenance-panel" @if ($classificationRunning) wire:poll.2s="pollWizardStepStatus" @endif>
                <header class="ff-maintenance-panel__header ff-maintenance-panel__header--muted">
                    <div class="min-w-0">
                ...
                <div class="ff-maintenance-panel__body space-y-4">
                    ...
                    @include('filament.tenant.partials.legacy-migration-step-results', [
                        'panel' => $classificationResultsPanel,
                    ])
        ...
        @if ($currentStep === 4)
            <section class="ff-maintenance-panel" @if ($classificationRunning) wire:poll.2s="pollWizardStepStatus" @endif>
                <header class="ff-maintenance-panel__header ff-maintenance-panel__header--muted">
                    <div class="min-w-0">
                        <h2 class="text-base font-semibold text-gray-900 dark:text-white">{{ __('Step 4: Classification file') }}
                        </h2>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
        ...
        @if ($currentStep === 5)
            ...
            @if ($lastRun)
                <details class="group rounded-xl border border-gray-200 bg-white shadow-sm dark:border-white/10 dark:bg-gray-900/60">
                    <summary class="cursor-pointer list-none px-4 py-3 sm:px-5 [&::-webkit-details-marker]:hidden">
                        <div class="flex items-center justify-between gap-2">
                            <div>
                                <h2 class="text-sm font-semibold text-gray-900 dark:text-white">{{ __('Last run result') }}</h2>
                                <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                                    {{ __('Full machine-readable summary of the last apply or dry run.') }}
                                </p>
                            </div>
                    ...
                    <div class="border-t border-gray-100 px-4 py-3 dark:border-white/10 sm:px-5">
                        <pre class="ff-ltr-data overflow-x-auto rounded-lg bg-gray-50 p-4 font-mono text-start text-xs dark:bg-gray-900/50"
                            dir="ltr">{{ json_encode($lastRun, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                    </div>
                </details>
            @endif
        @endif
