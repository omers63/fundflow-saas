    ...
    <div class="ff-system-maintenance mx-auto w-full space-y-8">
        ...
        <section class="space-y-6" aria-labelledby="ff-maintenance-purge-heading">
            <div class="ff-maintenance-danger-banner">
                ...
                <div class="min-w-0">
                    <h2 id="ff-maintenance-purge-heading" class="text-lg font-bold text-white">
                        {{ __('Purge database (destructive)') }}
                    </h2>
                    <p class="mt-1 text-sm text-red-100">
                        {{ __('Purge removes') }}
                        <strong class="text-white">{{ __('all rows') }}</strong>
                        {{ __('from every table that does') }}
                        <strong class="text-white">{{ __('not') }}</strong>
                        {{ __('have a') }}
                        <code class="rounded bg-white/20 px-1 text-xs">deleted_at</code>
                    ...
                    <header class="ff-maintenance-panel__header ff-maintenance-panel__header--muted">
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                {{ __('Never truncated by this tool.') }}</p>
                        </div>
                    </header>
                    <div class="ff-maintenance-panel__body">
                ...
                <div class="ff-maintenance-panel">
                    <header class="ff-maintenance-panel__header ff-maintenance-panel__header--muted">
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                {{ __('Not included in purge while this column exists.') }}</p>
                        </div>
                    </header>
                    <div class="ff-maintenance-panel__body">
                        <div class="ff-maintenance-scroll max-h-48">
                            @if (count($softDeleteSkippedTables) === 0)
                                <p class="text-xs text-gray-500 dark:text-gray-400">
                                    {{ __('None of your application tables currently use soft deletes.') }}</p>
                            @else
                                <ul class="space-y-1">
