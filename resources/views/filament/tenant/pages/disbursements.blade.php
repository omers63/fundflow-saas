<x-filament-panels::page>
    ...
    <div class="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        @foreach ([
            ...
            <div
                class="min-w-0 rounded-xl border border-gray-200 bg-white px-3 py-2.5 shadow-sm dark:border-white/10 dark:bg-slate-800">
                <p class="break-words text-[10px] font-semibold uppercase tracking-wider text-gray-500">{{ $stat['label'] }}</p>
                <p class="mt-1 break-words text-lg font-bold tabular-nums text-gray-900 dark:text-white">{{ $stat['value'] }}
                </p>
                </div>
        @endforeach
    </div>
