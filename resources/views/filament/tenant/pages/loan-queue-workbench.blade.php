...
$money = fn($amount) => \App\Filament\Support\MoneyDisplay::format((float) $amount, $currency, precision: 0) ?? number_format((float) $amount);
    ...
    ['label' => __('Disbursable now'), 'value' => $money($kpis['disbursable']), 'sub' => __('Across tier pools'), 'accent' => $kpis['disbursable'] > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-500'],
    ['label' => __('Ready to process'), 'value' => (string) $kpis['process'], 'sub' => trans_choice(':count emergency in queue|:count emergencies in queue', $kpis['emergency'], ['count' => $kpis['emergency']]), 'accent' => $kpis['process'] > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-500'],
                ...
                >
                    <summary class="flex cursor-pointer flex-wrap items-center justify-between gap-2 px-3 py-2">
                        ...
                        <span class="text-xs text-gray-500">
                            {{ __('Allocated :allocated · Committed :committed · Payable now :disbursable', [
