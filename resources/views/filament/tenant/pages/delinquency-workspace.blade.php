        ...
        @if ($showOverviewExtras)
            ...
            $snapshot = $this->insightsSnapshot();
                ...
                'title' => __('Overview'),
                    ...
                    <x-ff-lazy-fold section="value_chart_aging" :unfolded="$this->isSectionUnfolded('value_chart_aging')" :title="__('Value chart · Delinquency aging')" :hint="__('Expand to load overdue installment age buckets (cached).')">
                        @if ($this->isSectionUnfolded('value_chart_aging'))
                            @include('filament.partials.insights.value-chart', [
                                'chart' => app(\App\Services\ValueChartsService::class)->delinquencyAging(),
                            ])
                        @endif
                    </x-ff-lazy-fold>
                        ...
                        <div class="rounded-xl border border-gray-200 bg-white px-4 py-3 dark:border-white/10 dark:bg-slate-800">
                            ...
                            @if ($lastRun)
                                                                                                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-300">
                                                                                                            {{ __('Last run :at — :summary', [
                    'at' => \Illuminate\Support\Carbon::parse($lastRun['at'])->timezone(config('app.timezone'))->toDayDateTimeString(),
                    'summary' => \App\Services\Loans\LoanDelinquencyService::formatMaintenanceSummary($lastRun['result'] ?? []),
                ]) }}
                                                                                                        </p>
                            @else
