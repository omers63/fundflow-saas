@php
    ...
    $onPublicTenantRoute = request()->routeIs('tenant.*');
    $shouldTeleport = $isVisibleOutsidePanels
        || $onPublicTenantRoute
        || (
            !str_contains($resolvedRenderHook, '::sidebar.')
            && !str_contains($resolvedRenderHook, 'user-menu.')
        );
@endphp

<x-filament::dropdown :teleport="$shouldTeleport" :placement="$placement" :shift="true" :width="$isFlagsOnly ? 'w-fit fls-flag-only-width' : 'w-fit fls-dropdown-width'" :max-height="$maxHeight"
    class="fi-dropdown fi-user-menu language-switch-dropdown" data-nosnippet="true">
    <x-slot name="trigger">
        <div @class([
            ...
            @if ($isFlagsOnly || $hasFlags)
                <x-language-switch::flag :src="$languageSwitch->getFlag(app()->getLocale())" :circular="$isCircular"
                    :alt="$languageSwitch->getLabel(app()->getLocale())" :switch="true" />
            @else
    ...
    <x-filament::dropdown.list @class(['language-switch-dropdown__list !border-t-0 !p-1.5'])>
        @foreach ($locales as $locale)
            @if (!app()->isLocale($locale))
                <button type="button" wire:click="changeLocale('{{ $locale }}')" @if ($isFlagsOnly) x-tooltip="{
                    ...
                    @if ($isFlagsOnly)
                        <x-language-switch::flag :src="$languageSwitch->getFlag($locale)" :circular="$isCircular"
                            :alt="$languageSwitch->getLabel($locale)" class="w-7 h-7" />
                    @else
                        @if ($hasFlags)
                            <x-language-switch::flag :src="$languageSwitch->getFlag($locale)" :circular="$isCircular"
                                :alt="$languageSwitch->getLabel($locale)" class="w-7 h-7" />
