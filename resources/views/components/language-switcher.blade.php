...
@if (count($locales) > 1)
    <div x-data="{ open: false }" @click.outside="open = false" @keydown.escape.window="open = false"
        ...
        <button type="button" class="language-switch-trigger"
            data-language-tooltip="{{ $languageSwitch->getLabel($currentLocale) }}" @click="open = ! open"
            :aria-expanded="open.toString()" aria-haspopup="true" aria-controls="language-switch-menu"
            ...
            @foreach ($locales as $locale)
                @if (!app()->isLocale($locale))
                    <a href="{{ \App\Support\LocaleSwitchUrl::for($locale) }}" role="menuitem" class="language-switch-option"
                        data-language-tooltip="{{ $languageSwitch->getLabel($locale) }}" @click="open = false">
                        <img src="{{ $languageSwitch->getFlag($locale) }}" alt="{{ $languageSwitch->getLabel($locale) }}"
                            class="language-switch-option__flag" width="28" height="28" loading="lazy" decoding="async" />
                    </a>
                @endif
            @endforeach
        </div>
