...
@if (count($locales) > 1)
    <div x-data="{ open: false }" @click.outside="open = false" @keydown.escape.window="open = false"
        ...
        <button type="button" class="language-switch-trigger language-switch-trigger--labeled"
            ...
            <span class="language-switch-trigger__label">{{ $languageSwitch->getLabel($currentLocale) }}</span>
        </button>
        ...
        <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-100"
            ...
            @foreach ($locales as $locale)
                @if (! app()->isLocale($locale))
                    <a href="{{ \App\Support\LocaleSwitchUrl::for($locale) }}" role="menuitem"
                        class="language-switch-option language-switch-option--labeled" @click="open = false">
                        <img src="{{ $languageSwitch->getFlag($locale) }}" alt=""
                            class="language-switch-option__flag" width="28" height="28" loading="lazy"
                            decoding="async" />
