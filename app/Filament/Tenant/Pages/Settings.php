...
class Settings extends Page implements HasForms
{
    ...
    protected function defaultSettingsFormState(): array
    {
        ...
        return [
            ...
            'member_number_format' => $memberNumber['format'],
        ...
        return [
            ...
            'collection::tab' => [
                Section::make(__('Automation schedule'))
                    ->description(__('Cycle clocks, job times, behaviour toggles, and automation notifications live under Audit & System → Automation → Schedule.'))
                    ->schema([
                        Placeholder::make('automation_schedule_link')
                            ->hiddenLabel()
                            ->content(new HtmlString(
                                '<a href="'.e(AuditSystemTabRegistry::url('jobs', ['jobsTab' => 'schedule'])).'" class="text-sm font-medium text-primary-600 hover:underline dark:text-primary-400">'
                                .e(__('Open automation schedule'))
                                .'</a>'
                            )),
                    ]),
    ...
    public function save(): void
    {
        ...
        MemberNumberSettings::save([
            'format' => $state['member_number_format'] ?? $memberNumber['format'],
            'prefix' => $state['member_number_prefix'] ?? $memberNumber['prefix'],
            'separator' => $state['member_number_separator'] ?? $memberNumber['separator'],
            'padding' => (int) ($state['member_number_padding'] ?? $memberNumber['padding']),
     ...
     * @return list<string>
     */
    private static function memberNumberPreviewPartialTargets(): array
    {
        return [
            'member_number_prefix',
            'member_number_separator',
            'member_number_padding',
            'member_number_include_year',
            'member_number_preview',
        ];
    }
