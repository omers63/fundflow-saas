        ...
        return ViewFundPostingAction::configure($table)
            ...
            ->recordActions([
                ...
                Action::make('accept')
                    ...
                    ->modalHeading(__('Accept Fund Posting'))
                    ->modalWidth('2xl')
                    ...
                    ->action(function ($record, array $data, FundPostingService $service) {
                        ...
                        Notification::make()->title(__('Fund posting accepted'))->success()->send();
                    }),
                Action::make('reject')
                    ->label('Reject')
                    ...
                    ->color('danger')
                    ->requiresConfirmation()
                    ->modalHeading(__('Reject Fund Posting'))
                    ->modalWidth('2xl')
                    ->fillForm(ViewFundPostingAction::fillFormFromRecord())
                    ->hidden(fn ($record) => $record->status !== 'pending')
                    ->schema(fn (): array => ViewFundPostingAction::modalSchemaWith([
            ...
            ->toolbarActions([
                BulkActionGroup::make([
                    BulkAction::make('acceptSelected')
                        ...
                        ->color('success')
                        ->requiresConfirmation()
                        ->action(function (Collection $records, FundPostingService $service) {
                            $count = 0;
