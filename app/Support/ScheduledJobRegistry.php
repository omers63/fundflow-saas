...
final class ScheduledJobRegistry
{
    ...
    public static function all(): array
    {
        return [
            ...
            self::job('loans:check-defaults', __('Loan delinquency check'), __('Overdue, delinquency, guarantor defaults, auto-transfer (also runs after each Apply loan repayments). Review under Operations → Delinquency.'), AutomationScheduleSettings::loanDefaultsScheduleLabel(), 'loans', false),
            self::job('loans:push-schedule-grace', __('Push loan schedule (grace)'), __('Manual: push unpaid EMIs forward N cycles and apply grace — leave Loan # blank for all eligible single-overdue loans, or enter a loan # to push that schedule'), __('Manual only (one-time)'), 'loans', false),
